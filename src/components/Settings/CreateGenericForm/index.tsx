...
const CreateGenericForm = (props) => {
  ...
  const handleSetDataMarkerFromMap = (v) => {
    setDataMarker(v);
    setchangedPin(true);
  };
    ...
    if (props.fields) {
      props.fields.forEach((field) => {
        if (props.generic === "cities" && field.id === "province") {
          ...
          props.generic === "schools" &&
          ["country", "province", "city"].includes(field.id)
        ) {
          ...
          values[field.id] = props.data
            ? props.data.courses.map((course) => course._id)
            : "";
        } else if (props.generic === "schools" && "latitude" === field.id) {
          values[field.id] = "50.8563154";
        } else if (props.generic === "schools" && "longitude" === field.id) {
          values[field.id] = "-130.2052742";
        } else {
  ...
  useEffect(() => {
    if (
      dataMarker.lat !== formik?.values["latitude"] &&
      dataMarker.lng !== formik?.values["longitude"] &&
      changedPin
    ) {
  ...
  }, [dataMarker.lat, dataMarker.lng, changedPin]);
  ...
  const verifyIcon = () => {
    if (!!formik.values["icon"]) {
      if (formik.values["icon"]?.name) {
        ...
        if (
          formik.values["icon"].name.includes(".png") === false &&
  ...
  useEffect(() => {
    const findCity = props.cities.find(
      (city) => city._id === formik.values.city
    );
      ...
      {props.fields.map((field) => {
        if (props.generic === "cities" && field.id === "province") {
          return (
            <InputContainer
              key={field.id}
              label={field.label}
              labelClass={classNames({ "p-error": isFormFieldValid(field.id) })}
            >
              <select
                id={field.id}
                ...
                className={classNames({
                  "p-invalid": isFormFieldValid(field.id),
                })}
              >
                <option value=''></option>
            ...
            >
              ...
              <InputContainer
                ...
                labelClass={classNames({
                  "p-error": isFormFieldValid(field.id),
                })}
              >
                <Map
                  setDataMarker={handleSetDataMarkerFromMap}
                  position={{
                    lat: dataMarker.lat || 0,
                    lng: dataMarker.lng || 0,
                  }}
                  options={mapOptions}
                  iconType='school'
                />
        ...
        if (props.generic === "schools" && field.id === "city") {
          const filteredCities = props.cities?.filter(
            (ct) => ct.province === formik.values["province"]
          );
        ...
        if (props.generic === "schools" && field.id === "type") {
          return (
            ...
            >
              ...
              >
                <option value=''></option>
                {["Elementary School", "High School"].map((type) => (
          ...
          return (
            ...
            >
              ...
              <div style={{ display: "flex", marginTop: "12px" }}>
                <p>Or</p>
                <Button
                  onClick={handleImportIcon}
                  className='p-button-text p-mx-0'
                >
