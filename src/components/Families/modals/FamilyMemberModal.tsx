...
const FamilyMemberModal: React.FC<Props> = ({
  memberData,
  closeDialog,
  familyData,
  relationships,
  ...
  useEffect(() => {
    (async () => {
      const { genders, languages } = await GenericsService.getAll(
        session?.token,
        ["genders", "occupations", "languages"]
      );
  ...
  const formik = useFormik({
    ...
    onSubmit: (data) => {
      ...
      if (!memberData || memberData.familyRelationship === undefined) {
        ...
        const item = newMember.familyMembers.find(
