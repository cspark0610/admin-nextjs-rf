...
const BedroomModal : React.FC<Props>= ({data, onSubmit}) => {
    ...
    useEffect(() => {
      (async () => {
          const { additionalRoomFeatures } = await GenericsService.getAll(session?.token, ['additionalRoomFeatures'])
    ...
    return (
        <form onSubmit={formik.handleSubmit}>
          <InputContainer label="Photo" style={{paddingLeft: photoUrl ? '4rem' : '0'}}>
              {photoUrl ? 
           ...
           <InputContainer label="Availability" labelClass={classNames({ 'p-error': isFormFieldValid('availability') })}>
                <Calendar 
                    placeholder='Availability'
                    id='availability'
                    name='availability'
                    value={new Date(formik.values.availability)}
            ...
            <InputContainer label="Bathroom Location" labelClass={classNames({ 'p-error': isFormFieldValid('bathroomLocation') })}>
                <Dropdown
                  id="bathroomLocation"
                  ...
                  onChange={formik.handleChange}
                  className={classNames({ 'p-invalid': isFormFieldValid('bathroomLocation') })}
                />
                {getFormErrorMessage('bathroomLocation')}
            </InputContainer>
            <InputContainer label="Bath Type" labelClass={classNames({ 'p-error': isFormFieldValid('bathType') })}>
                <Dropdown
                  id="bathType"
                  options={BathTypes}
            ...
            <InputContainer label="Bed Type" labelClass={classNames({ 'p-error': isFormFieldValid('bedType') })}>
                <Dropdown
                  id="bedType"
                  options={BedTypes}
            ...
            <InputContainer label="Floor" labelClass={classNames({ 'p-error': isFormFieldValid('floor') })}>
                <Dropdown
                  id="floor"
                  options={FloorTypes}
                  placeholder="Bed Type" 
            ...
            <InputContainer label="Aditional Features" labelClass={classNames({ 'p-error': isFormFieldValid('aditionalFeatures') })}>
                <MultiSelect
