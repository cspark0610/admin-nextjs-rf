...
const TenantsModal: React.FC<Props> = ({
  tenantData,
  familyData,
  closeDialog,
}) => {
  ...
  useEffect(() => {
    ;(async () => {
      const { genders, occupations } = await GenericsService.getAll(
        session?.token,
        ['genders', 'occupations']
      )
  ...
  const formik = useFormik({
    ...
    onSubmit: (data) => {
      ...
      if (tenantData) {
        const updateTenant = tenants.find(
          (tenant) => tenant._id === tenantData._id
        )
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      <InputContainer
        label='First Name'
        labelClass={classNames({ 'p-error': isFormFieldValid('fisrtName') })}
      >
        <InputText
          id='firstName'
      ...
      <InputContainer
        label='Occupation'
        labelClass={classNames({ 'p-error': isFormFieldValid('occupation') })}
      >
        <Dropdown
          id='occupation'
