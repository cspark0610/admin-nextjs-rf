...
const HomePicturesForm = ({
  ...
  const handleDelete = async (data) => {
    ...
    if (updatedData.length === 0) {
      ...
      await FamiliesService.updateFamilyHome(session?.token, family._id, {
        photoGroups: [
          ...family.home.photoGroups.filter((pgf) => pgf.name !== homeCategory),
          {
            ...family.home.photoGroups.find((pg) => pg.name === homeCategory),
            photos: [],
          },
        ],
      })
      ...
      pictures.forEach((_, index) => {
        ...
        formData.delete(
          `photoGroups[${actualIndex}][photos][${index}][caption]`
        );
      });
