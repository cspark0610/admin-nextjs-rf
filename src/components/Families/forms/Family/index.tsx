...
export default function FamilyForm() {
  ...
  const [showExternalStudentsModal, setShowExternalStudentsModal] =
    useState(false);
  ...
  const familyMembers = useMemo(
    () =>
      family?.familyMembers?.map((member) => ({
        ...
        situation:
          member?.situation && member?.situation === "true"
            ? "Yes"
            : member?.situation === "false"
            ? "No"
            : member?.situation,
        _id: member?._id,
        familyRelationship:
    ...
    () =>
      family?.noRedLeafStudents?.map(
        ({
          ...
          lengthToStay: `${dateToDayAndMonth(
            stayingSince
          )} to ${dateToDayAndMonth(stayingUntil)}`,
          _id,
        })
      ),
    [family]
  ...
  const tenants = useMemo(
    () =>
      family?.tenantList?.map(
        ({ _id, firstName, lastName, gender, birthDate, occupation }) => ({
          firstName,
          lastName,
          gender: gender.name,
  ...
  useEffect(() => {
    (async () => {
      const { genders, familyRules, local_manager, program } =
  ...
  const handleEditData = (data, context) => {
    ...
    if (context === editContext.EXTERNAL_STUDENT)
      setShowExternalStudentsModal(true);
  ...
  const handleDeleteMany = (deleteData, context) => {
    confirmDialog({
      ...
      accept: () => {
        const newData =
          context === editContext.SCHOOLS
            ? family[arrayDataContent[context]].filter(
                (item) => !deleteData.includes(item.school._id)
              )
            : family[arrayDataContent[context]].filter(
